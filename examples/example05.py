...
"""
example05.py
A feature-expanded copy of example04 that adds:
  • Equivalent analytical system failure rate (λ_eq) from the analytical R(t)
    and the small‑time (t→0) slope.
  • Criticality importance in addition to Birnbaum & Fussell–Vesely.
  • Per‑component availability time‑series (with 95% Wilson bands), like the system.
  • Sensitivity simulation that multiplies one BE’s failure rate by (x1..x5) and
    plots system unavailability vs. the factor — one plot per BE.

Notes
-----
This script keeps the “extract from fault logs” approach from example04, and takes
inspiration from the exact R(t) construction and importance definitions used in
your other model (main05 / critical_rank / sensitivity_plots).

Usage
-----
python example05.py  # runs a demo against "fault_logs_example.csv" if present

Key outputs (written to ./output/):
  - availability_system.png
  - availability_components.png
  - sensitivity/BE*_sensitivity.png (one image per BE)
  - CSVs for per-component availability time series and sensitivity tables
"""
...
class FaultTreeFromLogs05:
    ...
    def extract_fault_tree(self) -> List[Set[str]]:
        """Extract minimal cut sets from time-ordered fault logs (same logic as example04)."""
        failure_states = set()
    ...
    def _p_TE_given_p(self, p_fail: Dict[str, float]) -> float:
        ...
        using a dynamic recursion over the minimal cut sets, identical in spirit to main05.
        """
