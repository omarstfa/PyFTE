...
class FaultTreeFromLogs:
    ...
    def plot_analysis(self, results: dict):
        ...
        # Plot 2: Analytical vs Monte Carlo Reliability
        analytical_R = [self.analytical_reliability(t) for t in time_grid]
        ax2.plot(time_grid, analytical_R, 'g--', linewidth=2, label='Analytical R(t)')
        ax2.plot(time_grid, results['reliability'], 'b-', linewidth=1, label='Monte Carlo R(t)')
        ax2.set_xlabel('Time (hours)')
        ax2.set_ylabel('Reliability')
        ax2.set_title('Analytical vs Monte Carlo Reliability')
        ax2.legend()
        ...
        # Plot 4: Importance Factors
        importance = self.calculate_importance_factors()
        components = list(importance.keys())
        birnbaum_values = [importance[be]['birnbaum'] for be in components]
        
        bars = ax4.bar(components, birnbaum_values, color='orange')
        ax4.set_xlabel('Component')
        ax4.set_ylabel('Birnbaum Importance')
        ax4.set_title('Component Importance Factors')
        
        # Add value labels on bars
        for bar, value in zip(bars, birnbaum_values):
            height = bar.get_height()
...
def run_complete_analysis(fault_logs_path: str):
    ...
    print(f"Most Critical Component: {most_critical}")
