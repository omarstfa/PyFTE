...
def safe_event_sort_key(col):
    if col == "TE":
        return (1, float('inf'))  # Push TE to the end
    num = extract_event_number(col)
    return (0, int(num)) if num.isdigit() else (0, float('inf'))

df_simple = df_simple[[c for c in sorted(df_simple.columns, key=safe_event_sort_key)]]
