...
truth_table_original = tree.generate_truth_table()
...
for mcs in minimal_cut_sets:
    print(sorted(mcs))
print("\nExtracted Fault Tree Boolean Expression: TE =", boolean_expression)
...
#         if len(factors) == 1:
#             # Single basic event
#             children.append(Node(factors[0], node_type="BASIC"))
#         else:
#             # AND gate with multiple children
#             and_children = [Node(f.strip(), node_type="BASIC") for f in factors]
...
#         local_vars = {var: val for var, val in zip(variables, combo)}
#         expr_eval = expr
#         for var, val in local_vars.items():
#             expr_eval = re.sub(rf'\b{var}\b', str(val), expr_eval)
#         result = eval(expr_eval.replace('+', ' or ').replace('.', ' and '))
#         table.append([local_vars[var] for var in variables] + [int(result)])
...
# # Part 2: Truth table
# print("\n Truth Table:")
# truth_table = generate_truth_table_from_expression(boolean_expression)
# print(truth_table.head())

# Optionally save it
# truth_table.to_csv("truth_table_from_expression.txt", sep=' ', index=False)
...
if constructed.equals(original):
    ...
    print("\n[Validation Failed]: Truth Tables Do Not Match!\n\nTDifferences detected between the constructed and original fault trees.")
